...
3. **A linha do tempo.** As 4 decisões numa escala de 0 a 40 anos, como no slide: Celular e Televisão desde o ano 0, Poupança no ano 8 e Herança no ano 15. Tocar numa linha leva até a decisão, e as decisões concluídas ganham um selo verde.

4. **Fazer os cálculos.** Cada decisão é um cartão com a pergunta do slide e as duas lado a lado, como as colunas da planilha: o retrato, o que cada uma escolheu, o texto do slide e as linhas da planilha. As linhas laranja já vêm preenchidas; nas brancas, o aluno digita o resultado. Embaixo, em verde, vem "Calcule a diferença entre as duas".

   Numa célula vale o número do jeito brasileiro (`1800`, `1.800,00`) ou a conta, como numa célula da planilha (`=4500*40%`, `0,4 x 4500`, `(3000+1050)*300`). Quando é uma conta, a célula mostra embaixo quanto ela dá. O "R$" e a unidade ("trocas", "meses") já estão desenhados em volta da célula. O Enter leva à próxima célula em aberto e, na última, confere.

5. **Conferir.** O botão **Conferir os cálculos** confere a decisão toda:

   - **certo:** a célula fica verde, com ✓, e trava mostrando o valor do jeito da planilha;
   - **errado:** fica vermelha e ganha uma dica com a conta que tem que ser feita ("Dica: Perda por troca de telefone × Quantidade de trocas no período");
   ...
   Depois da primeira conferência, aparece **Mostrar as respostas**, que preenche as células que ainda faltam (em roxo, com "Resposta mostrada").

   Quando todas as células da decisão estão certas ou mostradas, a decisão fica concluída. Aí aparece a pergunta do slide ("Por que a escolha do iPhone x Samsung deu uma diferença de R$ 26.800,00?"), com uma resposta possível escondida em **Ver uma resposta**, pra turma discutir antes.

   Enquanto o aluno desce pelas decisões, dois medidores ficam grudados no alto da tela: quantos dos 24 cálculos ele acertou (a parte mais clara da barra são as respostas mostradas) e quantas decisões já concluiu.

6. **A diferença total.** O último cartão é a última célula da planilha. Ele mostra a diferença de cada decisão já resolvida e pede a soma. Só dá pra conferir com as 4 decisões concluídas. Com a diferença total certa (ou mostrada), aparece a diferença final. Ela traz o número da aula, R$ 2.054.300,00 (que conta de zero até o valor), o resultado da Vanessa e o da Karine e quantos cálculos o aluno acertou. Depois vem uma linha por decisão com quanto da diferença final veio dela, o que os números mostram e as perguntas do slide de reflexão.
...
| --- | --- |
| Conferir a diferença total com decisão em aberto | Aviso no pé da tela: "Termine os cálculos das 4 decisões antes de calcular a diferença total. Falta a Decisão 3." |
| Ver as respostas antes de tentar | Não dá: o botão **Mostrar as respostas** só aparece depois da primeira conferência daquela decisão. |
| Mudar uma célula certa ou mostrada | Não dá: ela fica travada. Para fazer de novo, só recomeçando o exercício. |
...
frontend/
  ...
  js/sheet.js                   os cartões das decisões e o da diferença total, do jeito da
                                planilha, e os medidores do alto
...
Os dois arquivos em `backend/js/` não tocam no DOM: são só dados. O `exercise.js` e o `parse.js` também não. É o `exercise.js` que guarda o que o aluno fez, faz as contas, confere e aplica as regras; as telas só leem dele.
...
| As regras (quando dá pra conferir o total ou ver as respostas) | `blocked()` e `show()`, no `frontend/js/exercise.js` |
...
- **O exercício**, na chave `equilibrista-calculos`: o que o aluno digitou, as células resolvidas e as decisões já conferidas. Recarregar a página não apaga nada, e **Recomeçar o exercício** apaga tudo.
